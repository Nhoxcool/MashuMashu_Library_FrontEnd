...
export const Loans = () => {
  ...
  // Current Loans
  const [shelfCurrentLoans, setShelCurrentLoans] = useState<
    ShelfCurrentLoans[]
  >([]);
  ...
  useEffect(() => {
    const fetchUserCurrentLoans = async () => {
      if (authState && authState.isAuthenticated) {
        ...
        const shelfCurrentLoansReponse = await fetch(url, requestOptions);
        if (!shelfCurrentLoansReponse.ok) {
          throw new Error("Something went wrong!");
        }
        const shelfCurrentLoansResponseJson =
          await shelfCurrentLoansReponse.json();
        setShelCurrentLoans(shelfCurrentLoansResponseJson);
      }
